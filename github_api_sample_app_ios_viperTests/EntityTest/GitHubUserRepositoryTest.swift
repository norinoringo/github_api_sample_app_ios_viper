...
class GitHubUserRepositoryTest: XCTestCase {
    ...
    func testDecode() throws {
        ...
        let data = GitHubUserRepositoryTest.exampleJson.data(using: .utf8)!
        let githubUserRepository = try jsonDecoder.decode([GitHubUserRepositry].self, from: data)

        XCTAssertEqual(githubUserRepository[0].repositoryName, "30daysoflaptops.github.io")
        XCTAssertEqual(githubUserRepository[0].language, "CSS")
        XCTAssertEqual(githubUserRepository[0].stargazersCount, 7)
        XCTAssertEqual(githubUserRepository[0].description, nil)
        XCTAssertEqual(githubUserRepository[0].fork, false)
    }
}
